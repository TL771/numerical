...
import onepoint from '../Module/OnePoint';
import './OnePointIteration.css';
import functionPlot from "function-plot";
import {XYPlot, LineSeries,VerticalGridLines,HorizontalGridLines,XAxis,YAxis } from 'react-vis';
import StickyHeadTable from '../Table/Table';
export default class OnePointIteration extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            equation:'',
    ...
    findAnswer(){
        const ans = onepoint(this.state.equation,1);
        if(ans !== undefined){
            console.log(ans)
            this.setState({
                answer: ans.xn,
    ...
    onSelectionChanged(e){
        this.setState({equation:e.target.value,answer:undefined,statusAns:undefined,test:[],error:[]});
